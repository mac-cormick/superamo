...
Class AccountUpdateHandler {
    ...
    /**
     * AccountPaymentHandler constructor.
    ...
    public function handleAccountUpdate($data,$event) {
        ...
        if (isset($changedFields['pay_end'])) {
            ...
            $noteText .= sprintf(
                $this->langManager->get('subscription_date_changed'),
        ...
        if (isset($changedFields['tariff_id']) || isset($changedFields['paid_users'])) {
            ...
            if (is_null($prolongationOrder)) {
                ...
                if (isset($changedFields['paid_users'])) {
                    $noteText .= sprintf(
                        $this->langManager->get('users_number_changed'),
                        $oldPaidUsers,
                        $changedFields['paid_users']
                    );
                }
                ...
                if (isset($changedFields['tariff_id'])) {
                    $noteText .= sprintf(
                        $this->langManager->get('plan_changed'),
                        $oldTariffName,
        ...
        if (!empty($noteText)) {
            $note = [
                [
                    ...
                    'text' => json_encode([
                        'text' => $noteText,
                        'service' => $this->langManager->get('robot_name'),
                    ]),
