...
namespace CustomersBundle\Sync;

use Amo\Crm\GsoApiClient;
...
//	const NAME_LEAD = 'New Account: ';
	...
	/** @var GsoApiClient $_gso_api */
	protected $_gso_api;

	/** @var SupportApiClient $_supportApi */
	...
	protected $_statuses;
	protected $_fields;
	protected $_customers_account_id;
	...
	public function __construct(
		AmoCRM_API $api,
		GsoApiClient $gso_api,
		SupportApiClient $support_api,
		array $statuses,
		array $fields,
		...
		$this->_api = $api;
		$this->_gso_api = $gso_api;
		$this->_support_api = $support_api;
		$this->_statuses = $statuses;
		$this->_fields = $fields;
		$this->_logger = $logger;
		$this->_customers_account_id = $customers_account_id;
	 ...
	 * @return array
	 */
	public function user_update(array $data, array $contact = NULL, $only_return = FALSE)
	{
		$api = $this->_api;
		$logger = $this->_logger;
		$data_for_update = [];
...
//		$find_ru_account = FALSE;
		foreach($data['accounts'] as $account_info) {
			...
			$lead_name =  $this->set_lead_name($account_info['shard_type']);
...
//			}
			...
			$lead = [];
			$finded_leads = $api->find($api::ELEMENT_LEADS, $lead_name.$account_id);
			if(!empty($finded_leads)) {
				foreach($finded_leads as $key => $lead_info) {
					if($lead_info['name'] === $lead_name.$account_id) {
						$lead[] = $finded_leads[$key];
					}
				}
				if(!empty($lead)) {
					if(count($lead) > 1) {
						$logger->warning('Too many leads for one account', [$lead_name.$account_id]);
					}
					$lead = reset($lead);
					if($responsible_user_id == 0) {
...
//		if($find_ru_account) {
//			$this->_logger->info('find RU account');
//		} else {
//			$this->_logger->info('find only EN account');
//		}
...
//			}
		...
		if((int)$contact['id'] > 0) {
			...
			$this->_logger->info('add info to GSO');
			$human = [
				'contact_id'  => $contact['id'],
				'visitor_uid' => Uuid::uuid5(Uuid::NAMESPACE_OID, $data['id'])->toString(),
			];
			$this->_gso_api->set_human($human);
		}
	}
	...
	public function user_delete(array $data)
	{
		...
		foreach($data['accounts'] as $account_info) {
			...
			if(!empty($finded_leads)) {
				...
				if(!empty($lead)) {
					if(count($lead) > 1) {
						$logger->error('Too many leads for one account', [$lead_name.$account_id]);
					}
					$lead = reset($lead);
					$leads[] = $lead['id'];
				}
	...
	protected function get_tags(array $contact, $find_ru_account, array $data) {
		...
		$this->_logger->info('tags for add', $additional_info);
	...
	protected function handle_api_key_switch(array $data, LoggerInterface $logger, AmoCRM_API $api)
	{
		if (isset($data['flags']) && in_array(self::FLAG_API_RELATED_HASH_SWITCHED, $data['flags'])
			...
			} elseif (empty($manager = $this->_support_api->getAccountUser($this->_customers_account_id, $manager_id))) {
			...
			if (!$this->_api->add(AmoCRM_API::ELEMENT_NOTES, [$note])) {
				$this->_logger->info('Adding note error. ' . var_export($note, true));
			} else {
				$this->_logger->info('Note added.');
			}

			/**
			 * Проверим пользователей аккаунта и присвоим тег, если надо
			 */
			if (empty($users = $this->_support_api->getAccountUsers($account_id))) {
				...
				if ($update) {
					if ($this->_api->update(Entity_Element::get_entity_by_type($entity['type']), [$entity_data])) {
	...
	protected function set_lead_name($shard_type)
	{
		$lead_name = $shard_type === self::AMO_SHARD_TYPE_COM ? 'New Account: ' : 'Новый аккаунт: ';
		return $lead_name;
	}
}
